import serial
import time
from colorama import init
from termcolor import colored

# use Colorama to make Termcolor work on Windows too
init()

ser = serial.Serial()
ser.port = 'COM8'
ser.baudrate = 115200
ser.open()

#very large string allocate and deallocate test

#test 1

test1 = ser.readline()
print(test1)
if test1 == b'2A6V7W5NL5ZZC6AYE84NKZ6MVFMZ5DZSYD9TM3\r\n':
	print(colored('success test 1.1','blue'))
else:
	print(colored('fail test 1.1','red'))
test1 = ser.readline()
print(test1)
if(test1 == b'00000800\r\n'):
	print(colored('success test 1.2','blue'))
else:
	print(colored('fail test 1.2','red'))
test1 = ser.readline()
print(test1)
if test1 == b'\r\n':
	print(colored('success test 1.3','blue'))
else:
	print(colored('fail test 1.3','red'))

#test 2

character = ser.read(1)
print(character)
if character == b'A':
	print(colored('success test 2','blue'))
else:
	print(colored('fail test 2','red'))

#test 2

line = ser.readline()
print(line)
if line == b'this is not a line\r\n':
	print(colored('success test 3','blue'))
else:
	print(colored('fail test 3','red'))

#echo tests

test2_write = b'Hello my friend\r\n'
ser.write(test2_write)
test2_read = ser.readline()
print(test2_read)
if(test2_read == b'00000017\r\n'):
	print(colored('success echo test 1.1','blue'))
else:
	print(colored('fail echo test 1.1','red'))
test2_read = ser.readline()
print(test2_read)
if(test2_read == test2_write):
	print(colored('success echo test 1.2','blue'))
else:
	print(colored('fail echo test 1.2','red'))

test3_write = b'\r\n'
ser.write(test3_write)
test3_read = ser.readline()
print(test3_read)
if(test3_read == b'00000499\r\n'):
	print(colored('success echo test 2.1','blue'))
else:
	print(colored('fail echo test 2.1','red'))
test3_read = ser.readline()
print(test3_read)
if(test3_read == test3_write):
	print(colored('success echo test 2.2','blue'))
else:
	print(colored('fail echo test 2.2','red'))

test4_write = b'ramses\r\n'
ser.write(test4_write)
test4_read = ser.readline()
print(test4_read)
if(test4_read == b'00000008\r\n'):
	print(colored('success echo test 3.1','blue'))
else:
	print(colored('fail echo test 3.1','red'))
test4_read = ser.readline()
print(test4_read)
if(test4_read == test4_write):
	print(colored('success echo test 3.2','blue'))
else:
	print(colored('fail echo test 3.2','red'))

ser.write(test3_write)
test5_read = ser.readline()
print(test5_read)
if(test5_read == b'00000499\r\n'):
	print(colored('success echo test 5.1','blue'))
else:
	print(colored('fail echo test 5.1','red'))
test5_read = ser.readline()
print(test5_read)
if(test5_read == test3_write):
	print(colored('success echo test 5.2','blue'))
else:
	print(colored('fail echo test 5.2','red'))

test6_write = b'\r\n'
ser.write(test6_write)
time.sleep(1)
print(ser.in_waiting)
if(ser.in_waiting == 0):
	print(colored('success echo test 6.0','blue'))
else:
	print(colored('fail echo test 6.0','red'))
